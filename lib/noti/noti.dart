...
class _NotificationTestState extends State<NotificationTest> {
  ...
  void initState() {
    super
        .initState(); // 이 객체가 트리에 삽입 될 때 호출됩니다. 프레임 워크는 생성 된 각 [State] 객체에 대해이 메소드를 정확히 한 번만 호출합니다.
  ...
  Future _showNotification() async {
    var androidPlatformChannelSpecifics = AndroidNotificationDetails(
        'my-flutter-noti-id',
        'my-flutter-noti-name',
        'this is test noti in time');
    var iosPlatformChannelSpecifics = IOSNotificationDetails();
    var platformChannelSpecifics = NotificationDetails(
        androidPlatformChannelSpecifics, iosPlatformChannelSpecifics);

    await _flutterLocalNotificationsPlugin.show(
        0, 'Simple Noti', '기본 알림입니다.', platformChannelSpecifics,
        payload: 'Hello world!');
  }
